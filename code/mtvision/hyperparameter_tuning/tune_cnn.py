...
def make_objective(model_name: str, channels: str = "rgb"):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        lr = trial.suggest_float("lr", 1e-5, 5e-4, log=True)  # (can widen for ViT/ResNet)
        ...
        # -----------------------------
        # CONDITIONAL: imbalance hyperparameters only tuned for CNN
        # -----------------------------
        is_cnn = "cnn" in model_name.lower()
        is_pretrained = any(
            s in model_name.lower()
            for s in ["resnet", "vit"] 
        )

        if is_cnn:
            # High-level imbalance strategy for CNN
            ...
            alpha_ratio = BEST_ALPHA_RATIO

            # Strategy: simple random sampler + CE + class weights, no smoothing
            if imbalance_strategy == "random_ce_baseline":
                sampler_type = "random"
                loss_type = "ce"
                use_bias_init = False
                label_smoothing = 0.0

            # Strategy: random sampler + CE + tuned smoothing
            elif imbalance_strategy == "random_ce_smoothing":
                sampler_type = "random"
                loss_type = "ce"
                use_bias_init = False
            ...
            elif imbalance_strategy == "weighted_ce":
                sampler_type = "weighted"
                loss_type = "ce"
                use_bias_init = False
                label_smoothing = 0.0

            # Strategy: random sampler + focal loss (no class weights/smoothing)
            elif imbalance_strategy == "random_focal":
                sampler_type = "random"
                loss_type = "focal"
                use_bias_init = True
                label_smoothing = 0.0
                focal_gamma = trial.suggest_float("focal_gamma", 1.0, 5.0)
                alpha_ratio = trial.suggest_float("alpha_ratio", 1.0, 15.0)
        else:
            # NON-CNN MODELS → freeze imbalance configuration from best CNN
            sampler_type = BEST_SAMPLER_TYPE
            loss_type = BEST_LOSS_TYPE
            focal_gamma = BEST_FOCAL_GAMMA
            alpha_ratio = BEST_ALPHA_RATIO
            label_smoothing = 0.0
            # simple default until best config is known
            use_bias_init = sampler_type != "weighted"
        ...
        f"lr={lr:.3e}, wd={weight_decay:.3e}, dropout={dropout_p:.2f}, "
        f"bs={batch_size}, opt={optimizer_name}, sampler={sampler_type}, "
        f"aug={augmentation_mode}, loss={loss_type}, "
        f"gamma={focal_gamma}, alpha_ratio={alpha_ratio}, "
        ...
        for fold, (train_idx, val_idx) in enumerate(
            ...
            # Classifier bias init based on *train* priors
            if use_bias_init:
                init_classifier_bias_with_priors(model, class_counts)
            else:
                print("[INFO] Skipping classifier bias initialisation.")
...
def main():
    ...
    #  ex"cnn" (handled in: if model_name in ("cnn", "simplecnn")).
    model_name = "cnn"
