...
def make_objective(model_name: str, channels: str = "rgb"):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        is_vit = "vitsmall" in model_name.lower() or "ViTSmall_last4" in model_name.lower()

        if is_vit:
            # ViT-S/16 last4: slightly more conservative backbone LR
            lr_head = trial.suggest_float("lr_head", 1e-5, 3e-4, log=True)
            if model_name != "resnet18_whole_backbone":
                lr_last_blocks = trial.suggest_float("lr_last_block", 3e-6, 5e-5, log=True)
        else:
            # CNN / ResNet: your original ranges
            lr_head = trial.suggest_float("lr_head", 1e-5, 5e-4, log=True)
            if model_name != "resnet18_whole_backbone":
                lr_last_blocks = trial.suggest_float("lr_last_block", 1e-6, 1e-4, log=True)
        ...
        if is_vit:
            ...
            optimizer_name = "AdamW"  # fix for ViT
            augmentation_mode = trial.suggest_categorical(
                "augmentation_mode", ["basic"]  # always basic for ViT
            )
        else:
        ...
        for fold, (train_idx, val_idx) in enumerate(
            ...
            print(
                ...
                f"distribution (first {MAX_DIST_BATCHES} batches): {dict(dist)}"
                        ...
                        # ---- PARAM GROUPS: depends on model type ----
            if model_name == "resnet18_whole_backbone":
                # Fine-tune the entire backbone with a single LR
                param_groups = [
                    {"params": model.parameters(), "lr": lr_head},
                ]
                print("[INFO] Using single LR for full backbone")

            elif model_name.lower().startswith("resnet"):
                # ResNet-style: head (fc) with lr_head, last two blocks with lr_last_blocks
                param_groups = [
                ...
                print("[INFO] ResNet param groups: head + layer3+layer4")

            elif "vitsmall" in model_name.lower() or "ViTSmall_last4" in model_name.lower():
                # ViT-Small-last4:
                #   - classifier head: lr_head
                #   - unfrozen ViT blocks + layernorm: lr_last_blocks
                backbone_params = []
                head_params = []
                ...
                param_groups = [
                    ...
                    {"params": backbone_params, "lr": lr_last_blocks},
                ]

                print(
                    "[INFO] ViT param groups: "
                    f"{len(head_params)} head params @ lr_head, "
                    f"{len(backbone_params)} backbone params @ lr_last_blocks"
                )

            else:
                raise ValueError(f"Unknown model_name pattern for param_groups: {model_name}")
...
def main():
    ...
    # ---------------- OPTUNA STUDY CONFIG ----------------
    storage_path = "sqlite:///ViTSmall_last4_tuning.db"  
